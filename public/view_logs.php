            ...
            <div class="bg-transparent md:bg-white/90 md:backdrop-blur-sm md:rounded-2xl md:shadow-xl md:border md:border-white/50 overflow-hidden mb-10">
                ...
                <table class="min-w-full leading-normal">
                    ...
                    <tbody id="logsTableBody" class="block md:table-row-group space-y-4 md:space-y-0">
                        <?php
                        // --- FIX ADDED HERE: SET TIMEZONE ---
                        date_default_timezone_set('Asia/Manila'); 

                        // Initial Load (Server Side Rendering for first view)
                        $sql = "SELECT * FROM system_logs ORDER BY created_at DESC LIMIT 50";
                        $result = $conn->query($sql);

                        if ($result && $result->num_rows > 0) {
                            while($row = $result->fetch_assoc()) {
                                // Now date() will use Philippines time
                               // --- FIX: Convert UTC to Manila Time ---
        ...
        document.addEventListener('DOMContentLoaded', function() {
            ...
            function updateLogs() {
                ...
                .then(data => {
                    ...
                    // Update the Total Count if the element exists
                    const countDisplay = document.getElementById('totalEventsCount');
                    if (countDisplay && countDisplay.innerText !== data.count) {
